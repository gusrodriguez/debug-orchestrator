...
If `trace_start` is available as a tool, follow the tracing protocol at
`ai-agent-observer/prompts/tracing-protocol.md`. Trace every phase as a span, every
agent spawn as a child span, every HARD STOP as a checkpoint event, and every
escalation (Worker → Fixer → Specialist) as an escalation event.

If `trace_start` is not available, skip all tracing — it is optional.
